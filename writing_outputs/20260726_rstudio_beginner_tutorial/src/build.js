...
// 运行结果块：区别于代码块，用浅绿灰底 + 左侧灰条，并标注“运行结果”
function outParas(text) {
  const lines = text.split('\n');
  const head = new Paragraph({
    shading: { fill: 'EFF3EF', type: ShadingType.CLEAR },
    spacing: { before: 40, after: 0, line: 240 },
    indent: { left: 220, right: 220 },
    border: {
      left: { style: BorderStyle.SINGLE, size: 12, color: '7A9A7A', space: 6 },
      top: { style: BorderStyle.SINGLE, size: 2, color: 'DCE4DC', space: 2 },
    },
    children: [new TextRun({ text: '运行结果', font: FONT, size: 15, bold: true, color: '55705B' })],
  });
  const body = lines.map((line, i) => new Paragraph({
    shading: { fill: 'EFF3EF', type: ShadingType.CLEAR },
    spacing: { before: 0, after: i === lines.length - 1 ? 160 : 0, line: 250 },
    indent: { left: 220, right: 220 },
    border: {
      left: { style: BorderStyle.SINGLE, size: 12, color: '7A9A7A', space: 6 },
      ...(i === lines.length - 1 ? { bottom: { style: BorderStyle.SINGLE, size: 2, color: 'DCE4DC', space: 2 } } : {}),
    },
    children: [new TextRun({ text: line.length ? line : ' ', font: MONO, size: 17, color: '2A3A2A' })],
  }));
  return [head, ...body];
}
  ...
  switch (bk.t) {
    ...
    case 'warn':
      children.push(new Paragraph({
        ...
        border: {
          left: { style: BorderStyle.SINGLE, size: 12, color: 'C0504D', space: 6 },
          top: { style: BorderStyle.SINGLE, size: 2, color: 'F2CFCF', space: 4 },
          bottom: { style: BorderStyle.SINGLE, size: 2, color: 'F2CFCF', space: 4 },
          right: { style: BorderStyle.SINGLE, size: 2, color: 'F2CFCF', space: 4 },
        },
        children: [
          new TextRun({ text: '⚠ 注意　', font: FONT, size: 19, bold: true, color: 'A83A37' }),
          ...runsWithLinks(bk.x, { size: 19 }),
        ],
      }));
      break;
    case 'tbl':
    ...
    case 'note':
      children.push(new Paragraph({
        ...
        children: [
          new TextRun({ text: '💡 提示　', font: FONT, size: 19, bold: true, color: 'A5761B' }),
          ...runsWithLinks(bk.x, { size: 19 }),
        ],
      }));
      break;
  ...
  sections: [{
    ...
    headers: {
      default: new Header({ children: [new Paragraph({
        ...
        children: [new TextRun({ text: 'R 语言零基础入门教程 · Windows + RStudio · 2026-07', font: FONT, size: 16, color: '888888' })],
      })] }),
    },
    footers: {
...
const out = path.join(ROOT, 'final', 'R语言零基础入门教程_Windows_RStudio.docx');
